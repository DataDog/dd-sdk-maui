...
final class DdLogsTests: XCTestCase {
    ...
    func testLogDebug_beforeEnable_dropsLogAndDoesNotEnableModule() {
        // Simulate pre-enable state: clear the logger that setUp injected
        DdLogs.setLogger(nil)
        ...
        XCTAssertFalse(mockLogsModule.enableCalled)
        XCTAssertFalse(mockLogsModule.enableWithConfigCalled)
        // Logger must NOT have received any call
        XCTAssertEqual(mockLogger.logCalls.count, 0)
    }
}