# Symbol Upload for Crash Symbolication

The `Datadog.Maui` NuGet package includes an MSBuild target that automatically uploads debug symbols to Datadog after `dotnet publish`. This enables server-side symbolication of native crashes reported by the SDK.

## Prerequisites

Install `datadog-ci` globally:

```bash
npm install -g @datadog/datadog-ci
```

Export your Datadog API key:

```bash
export DATADOG_API_KEY=your-api-key
```

## What gets uploaded

| Platform | File | What it covers | Generated by |
|----------|------|---------------|-------------|
| **iOS** | `.dSYM` bundle | Native crashes + AOT-compiled C# method names | `dotnet publish -r ios-arm64` |
| **Android** | `mapping.txt` | R8/ProGuard obfuscated Java/Kotlin class and method names | `dotnet publish` with `AndroidLinkTool=r8` |
| **Both** | Portable PDB | Managed C# stack traces (file/line info), correlated by build-time debug ID manifest | `dotnet publish` (Debug symbols enabled) |

## Quick start

### iOS

```bash
dotnet publish -c Release -f net10.0-ios -r ios-arm64 \
  -p:DatadogUploadSymbols=true
```

### Android

```bash
dotnet publish -c Release -f net10.0-android \
  -p:DatadogUploadSymbols=true
```

## Configuration

All configuration is done via MSBuild properties. These can be set in your `.csproj` file or passed on the command line with `-p:`.

### MSBuild properties

| Property | Required | Default | Description |
|----------|----------|---------|-------------|
| `DatadogUploadSymbols` | Yes | `false` | Set to `true` to enable symbol upload after publish |
| `DatadogServiceName` | No | `$(AssemblyName)` | Service name used to identify your app in Datadog |
| `DatadogSite` | No | `datadoghq.com` | Datadog site (e.g., `datadoghq.eu`, `us5.datadoghq.com`) |
| `DatadogApiKey` | No | — | API key passed directly. If not set, the `DATADOG_API_KEY` environment variable is used instead |
| `DatadogKeepPortablePdbs` | No | `false` | iOS only. Some debug configurations (e.g. `soft-debug`) bundle the Portable PDB inside the `.app`; since symbolication is always done offline from the uploaded PDB, it's stripped from the bundle by default. Set to `true` to keep it in the bundle instead |

### Setting properties in `.csproj`

```xml
<PropertyGroup>
  <DatadogServiceName>my-app</DatadogServiceName>
  <DatadogSite>datadoghq.eu</DatadogSite>
</PropertyGroup>
```

### Setting properties on the command line

```bash
dotnet publish -c Release -f net10.0-ios -r ios-arm64 \
  -p:DatadogUploadSymbols=true \
  -p:DatadogServiceName=my-app \
  -p:DatadogSite=datadoghq.eu
```

## API key

The API key can be provided in two ways:

1. **Environment variable** (recommended for CI):
   ```bash
   export DATADOG_API_KEY=your-key
   dotnet publish -c Release -p:DatadogUploadSymbols=true ...
   ```

2. **MSBuild property** (useful for local testing):
   ```bash
   dotnet publish -c Release -p:DatadogUploadSymbols=true -p:DatadogApiKey=your-key ...
   ```

When using the environment variable, the shell's `DATADOG_API_KEY` passes through to `datadog-ci` automatically. When using the MSBuild property, it is injected into the `datadog-ci` process environment.

**Important:** Do not set `DatadogApiKey` in your `.csproj` file — it will be committed to source control. Use environment variables for API keys.

## Android: enabling R8

By default, .NET MAUI Android Release builds do not run R8, so no `mapping.txt` is generated. To enable it, add the following to your `.csproj`:

```xml
<PropertyGroup Condition="'$(Configuration)' == 'Release'">
  <AndroidLinkTool>r8</AndroidLinkTool>
  <AndroidCreateProguardMappingFile>true</AndroidCreateProguardMappingFile>
</PropertyGroup>
```

The mapping file will be generated at `bin/Release/net10.0-android/mapping.txt`.

## SDK framework symbols (DatadogWrapper)

The `DatadogWrapper` native framework (Swift/Kotlin) has its own debug symbols that are shared across all apps using the same SDK version. These are uploaded separately from app symbols:

```bash
./upload-sdk-symbols.sh
```

This uploads the `DatadogWrapper.framework.dSYM` (generated by `./build.sh`) to Datadog. Run this once per SDK release — all consumer apps benefit automatically.

The example app's `publish.sh` calls this automatically after uploading app symbols.

## How it works

The `Datadog.Maui` NuGet package ships a `.targets` file (`Datadog.Maui.targets`) under `buildTransitive/`. MSBuild automatically imports it into any project that references the package — no manual setup required.

The targets file defines four MSBuild targets that run after `dotnet publish`:

1. **`DatadogCheckCli`** — Verifies that `datadog-ci` is installed and the API key is available. If either check fails, a message is printed and upload is skipped.

2. **`DatadogUploadDsym`** (iOS) — Locates the app's `.dSYM` bundle next to the `.app` in the publish output and uploads it via `datadog-ci dsyms upload`. Note: this uploads only the app's dSYM. The `DatadogWrapper.framework` dSYM is uploaded separately during SDK release (see above).

3. **`DatadogUploadMapping`** (Android) — Locates `mapping.txt` in the build output and uploads it via `datadog-ci flutter-symbols upload --android-mapping`.

4. **`DatadogUploadPortablePdb`** (iOS + Android) — Uploads first-party Portable PDBs from the publish output directory via `datadog-ci ppdb-symbols upload`, correlated by the build-time debug ID manifest (`dd_debug_ids.json`) generated alongside the build.

5. **`DatadogSymbolReport`** — Prints a summary of what was uploaded (service, version, site, file paths).

### Verbosity

By default, `dotnet publish` uses the .NET terminal logger which hides informational messages. To see the Datadog upload output, add `-v n -tl:off`:

```bash
dotnet publish -c Release -f net10.0-ios -r ios-arm64 \
  -p:DatadogUploadSymbols=true -v n -tl:off
```

## Symbol file locations

After a Release publish, symbol files are located at:

### iOS
```
bin/Release/net10.0-ios/ios-arm64/
  example.app.dSYM/          <-- dSYM bundle (uploaded to Datadog)
  example.app/                <-- signed app bundle
  example.pdb                 <-- portable PDB (uploaded to Datadog)
  publish/
    example.ipa               <-- distributable package
```

### Android
```
bin/Release/net10.0-android/
  mapping.txt                 <-- R8 mapping (uploaded to Datadog, requires AndroidLinkTool=r8)
  example.pdb                  <-- portable PDB (uploaded to Datadog)
  publish/
    com.datadog.maui.test.apk <-- distributable package
```

Both Portable PDB uploads are correlated against `obj/<Configuration>/<TargetFramework>/dd_debug_ids.json`, a debug ID manifest generated at build time (see `Datadog.Maui.DebugId.targets`).

## Troubleshooting

**"Skipping symbol upload — datadog-ci is not installed"**
Run `npm install -g @datadog/datadog-ci` and verify with `datadog-ci version`.

**"Skipping symbol upload — DATADOG_API_KEY is not set"**
Export the key in your shell: `export DATADOG_API_KEY=your-key`. Verify with `echo $DATADOG_API_KEY`.

**"Skipping dSYM upload — file not found"**
dSYMs are only generated for device builds (`-r ios-arm64`). Simulator builds (`iossimulator-arm64`) do not produce dSYMs.

**"Skipping mapping upload — mapping.txt not found"**
Ensure R8 is enabled in your `.csproj` with `<AndroidLinkTool>r8</AndroidLinkTool>` and `<AndroidCreateProguardMappingFile>true</AndroidCreateProguardMappingFile>`.

**"Skipping Portable PDB upload — no debug-id manifest found"**
The debug ID manifest (`dd_debug_ids.json`) is generated during `Build`, which `Publish` depends on. If it's missing, check that `DebugSymbols` and `DebugType` are set (`<DebugType>portable</DebugType>`) and that the build itself succeeded.

**No Datadog output visible**
Add `-v n -tl:off` to your `dotnet publish` command to disable the terminal logger and show all build messages.

**Upload seems to work but no symbols appear in Datadog**
Symbols take up to 5 minutes to process after upload. Check **Error Tracking > Settings > Symbol Files** in Datadog.

## Known limitations

### Android NDK crash symbols

When `nativeCrashReportEnabled` is set to `true` in the RUM configuration, `dd-sdk-android-ndk` captures crashes in native (C/C++) code. These stack traces require unstripped `.so` files (ELF debug symbols) for symbolication — the R8 `mapping.txt` only covers Java/Kotlin obfuscation and does not help with native frames.

In a MAUI app, native `.so` files come from:
- **The .NET runtime** (`libmonosgen-2.0.so`, `libmonodroid.so`) — symbols are published by Microsoft on their symbol server, not generated during your build
- **Datadog's NDK library** (`libdatadog-ndk.so`) — Datadog resolves these server-side using their own published symbols
- **Custom native C/C++ libraries** — rare in MAUI apps, but if present their symbols would need manual upload via `datadog-ci dsyms upload <path-to-so-directory>`

The current automatic upload (mapping.txt + dSYM) covers the typical MAUI use case. If you see unsymbolicated native frames in NDK crash reports, the missing symbols likely belong to the .NET runtime and would need to be sourced from Microsoft's symbol server.
