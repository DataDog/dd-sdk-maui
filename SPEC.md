...
**One provider per dependency.** A dependency that has a NuGet binding is declared as a `PackageReference` and nothing else. Declaring it *also* as an `AndroidMavenLibrary` packages the Maven jar's classes into our AAR, where they sit outside NuGet's version resolution — a consuming app that pulls a different version of the same library then fails dex-merge with a duplicate-class error it has no clean way to fix. Only dependencies with no NuGet binding are packaged as `AndroidMavenLibrary` — of the artifacts tracked in `android-transitive-deps.json`, that is currently just kronos. In `android-transitive-deps.json` this is the invariant that `maven_library_csproj` and `package_ref_csproj` are never both non-empty for the same artifact.

Other `AndroidMavenLibrary` entries exist outside that file and are unaffected by the invariant, because none of them has a NuGet binding to conflict with: the `com.datadoghq:*` artifacts are the native SDK itself, and `org.jctools:jctools-core` and `com.google.re2j:re2j` (Trace) and `androidx.metrics:metrics-performance` (RUM) are Maven-only. Should any of them ever gain a NuGet binding, it must be added to `android-transitive-deps.json` and moved to a `PackageReference`.
...
- View tracking: subscribes to `Application.PageAppearing` (a single app-level event raised for any page appearing — Shell route changes, `Navigation.PushAsync`, modals) and calls `StartView`. Window-level `Resumed`/`Stopped` events drive view restart/stop on background/foreground transitions and are wired via `Application.DescendantAdded`.
  - View name resolution priority: `ViewNamePredicate` (if non-null) → `[DdView]` attribute on the page class → resolved Shell destination route → page class name.
  - `[DdView("Name")]` is a `DdViewAttribute` in the `Datadog.Maui` namespace. It can be applied to any `ContentPage` subclass to assign a fixed RUM view name, independent of Shell routing or class name.
  - Shell route resolution: when the appearing page is `Shell.Current.CurrentPage`, the destination route is computed in `Shell.Navigating` by resolving the (possibly relative) `Target.Location` (`..`, `../Other`, `DetailPage`) against the last `Shell.Navigated` location, so the name is correct even though `CurrentState.Location` lags `PageAppearing` by one nav step. Internal MAUI-generated routes (`D_FAULT_*`, `IMPL_*`, used for `PushAsync` inside Shell) are filtered out — those views fall back to the page class name. Non-Shell pages also use the page class name.
