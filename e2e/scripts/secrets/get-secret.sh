...
get_secret() {
    ...
    if [ "$CI" = "true" ]; then
        # Export AWS credentials explicitly for Vault (workaround for IMDSv2 + older Vault SDK)
    ...
    local secret_value=$(vault kv get -field=value "$DD_SDK_MAUI_SECRETS_PATH_PREFIX/$secret_name")
