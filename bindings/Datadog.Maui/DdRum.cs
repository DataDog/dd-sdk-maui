...
#if ANDROID
using NativeDdRum = Datadog.Android.Binding.DdRum;
#elif IOS
using Foundation;
using NativeDdRum = Datadog.iOS.Binding.DdRum;
#endif
                ...
                else
                {
                    if (configuration.AutomaticViewTracking)
                    {
                        InternalLog.Log(
                            ...
                            "Use builder.UseDatadogRum(config) from Datadog.Maui.Hosting, or call DdRum.Enable from a Page constructor.",
                            SdkVerbosity.WARN);
                    }
