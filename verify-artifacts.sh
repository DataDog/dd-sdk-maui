...
if [ "$RUN_NUGET" = true ]; then
    ...
    log_section "Datadog.Android.Binding NuGet structure"

    BINDING_PKG="local-packages/Datadog.Android.Binding.${SDK_VERSION}.nupkg"

    if [ -f "$BINDING_PKG" ]; then
        # Assembly must always be present
        assert_nupkg_contains \
            "lib/.../Datadog.Android.Binding.dll" \
            "$BINDING_PKG" \
            "lib/.*Datadog.Android.Binding.dll" || NUGET_RESULT=1

        # .targets file — injects ProGuard rules into consuming app builds
        assert_nupkg_contains \
            "buildTransitive/Datadog.Android.Binding.targets" \
            "$BINDING_PKG" \
            "buildTransitive/.*Datadog.Android.Binding.targets" || NUGET_RESULT=1
        ...
        log_warn "Skipping structure checks — Datadog.Android.Binding.${SDK_VERSION}.nupkg not found"
        NUGET_RESULT=1
    fi
...
if [ "$RUN_PROGUARD" = true ]; then
    ...
    CURRENT_RULES="bindings/Datadog.Android.Binding/Transforms/proguard.txt"
